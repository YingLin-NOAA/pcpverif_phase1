...
if [ $# -eq 0 ]; then   
  today=`date -u +%Y%m%d`
  vday=${vday:-`/nwprod/util/ush/finddate.sh $today d-1`}
else        
  vday=$1
fi          

vdaym1=`/nwprod/util/ush/finddate.sh $vday d-1`
COMOUT=/ptmpp1/Ying.Lin/verf.dat/precip
...
if [[ ($# = 0) && (($err1 = 0 && $err2 = 0) || $err3 = 0 ) ]]; then
  bsub < $VERF/ecf/jverf_precip_verfgen_24h.oconus.ecf > $CRONOUT/cron.verfgen.oconus.out 2>&1
fi 
