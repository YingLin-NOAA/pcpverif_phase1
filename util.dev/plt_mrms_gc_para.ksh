#!/bin/ksh 
set -x
# plot a 24h (12Z-12Z) total of gauge-corrected MRMS from /dcomdev
#
day=$1
...
DCOM=/dcomdev/us007003/ldmdata/obs/upperair/mrms/conus/GaugeCorr_QPE
...
do
  ...
  cp $DCOM/$mrmsfile.gz para.$mrmsfile.gz
  gunzip para.$mrmsfile
  err=$?
  if [ $err -ne 0 ]; then
    echo Problem copying or gunzip para.$mrmsfile
    allthere=N
  fi
...
cat > mrms_gc_para.ctl <<EOF1
dset ^para.GaugeCorr_QPE_01H_00.00_%y4%m2%d2-%h20000.grib2
index ^mrms_gc_para.idx
options template
undef 9.999E+20
title MRMS
...
cat > pltgrads_mrms_gc_para.gs <<EOF2
'open mrms_gc_para.ctl'
...
'draw title Para GC MRMS 24h pcp ending 12Z ${day}'
'printim mrms_gc_para.${day}12.24h.gif gif x480 y425 white'
'quit'
EOF2

gribmap -i mrms_gc_para.ctl

grads -blc "run pltgrads_mrms_gc_para.gs"
exit
